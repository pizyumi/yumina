...
obj = _.extend(obj, {
  initialize: async (env, p_work, p_yumina) => {
    ...
    await common.create_folder_from_path(con.p_temp);
    await common.create_folder_from_path(con.p_fstemp);
    await common.create_folder_from_path(con.p_fspublic);
...
async function compile_libcss (con) {
  ...
  var p1 = _(p_css_sys).map((v) => path.join(con.p_yasset, v));
  var p2 = _(p_css_yumina).map((v) => path.join(con.p_front, v));
  var p3 = _(p_css).map((v) => path.join(con.p_design, v));
  var p_in = _.union(p1, p2, p3, con.p_css);
    ...
    if (role.auto_generate_web) {
      ...
      await pi.forEachSeries(_.values(e.get_entities()), async (v) => {
        ...
        if (auth.list.includes(rname)) {
          items.push({
            name: pname,
            disp_en: entity.disp_en,
            disp_ja: entity.disp_ja,
            url: rurl + pname
          });
        }
      });
...
async function compile_item (con, e, r, p_base, ename) {
  ...
  var sjs = await common.load_text_from_path(p_logic);
  var template = `
  var _ = require('underscore');

  <%- sjs %>

  module.exports = <%- ename %>_logic;
  `;
  var js = ejs.render(template, { ename: ename, sjs: sjs });
  var p_js = path.join(con.p_temp, ename + '.js');
  await common.save_text_to_path(p_js, js);

  var logic = require(path.join(process.cwd(), p_js));

  await common.remove_file_from_path(p_js);

  var p_component = _.uniq(_.flatten(_(logic.item_schema).map((v) => {
    if (v.type === 'text') {
      return [path.join(con.p_front, 'component', 'input-text.js')];
    }
    else if (v.type === 'integer') {
      return [path.join(con.p_front, 'component', 'input-integer.js')];
    }
    else if (v.type === 'integer-range') {
      return [
        path.join(con.p_front, 'component', 'input-integer.js'),
        path.join(con.p_front, 'component', 'input-integer-range.js')
      ];
    }
    else if (v.type === 'date') {
      return [
        path.join(con.p_front, 'component', 'input-integer.js'),
        path.join(con.p_front, 'component', 'input-date.js')
      ];
    }
      ...
      return [
        path.join(con.p_front, 'component', 'input-integer.js'),
        path.join(con.p_front, 'component', 'input-date.js'),
        path.join(con.p_front, 'component', 'input-date-range.js')
      ];
    }
    else if (v.type === 'select') {
      return [path.join(con.p_front, 'component', 'select-one.js')];
    }
    else {
...
async function compile (p_in, p_out) {
  var compiled = (await pi.mapSeries(p_in, async (v) => {
    var text = await common.load_text_from_path_empty(v);

    if (text === '') {
      console.log((v + ' is empty.').magenta);
    }
