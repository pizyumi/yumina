...
obj = _.extend(obj, {
  ...
  compile: async (con, e) => {
    ...
    await pi.forEachSeries(_.values(e.get_entities()), async (v) => {
      ...
      await compile_item(con, p_base, ename);
    });
  }
});
...
async function compile_page (con, p_base, pname) {
...
async function compile_item (con, p_base, ename) {
  var ptype = 'item';

  var p_menu = path.join(con.p_design, 'menu.js');
  var p_main = path.join('front', 'main.js');
  var p_entity = path.join('front', 'component', 'entity.js');
  var p_form_item = path.join('front', 'component', 'form-item.js');
  var p_logic = path.join(p_base, 'logic.js');
  var p_page = path.join(p_base, ptype, 'page.js');
  ...
  var p_component = _.uniq(_(logic.item_schema).map((v) => {
    if (v.type === 'text') {
      return path.join('front', 'component', 'input-text.js');
    }
    else {
