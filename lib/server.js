          ...
          rrole.get('/' + pname, (req, res, next) => {
            co(function* () {
              if (_.result(req.query, 'action', '') === '') {
                yield send_res(req, res, name, 'list', data, title, con);
              }
              else {
                next();
              }
        ...
        if (auth.item.includes(rname)) {
          ...
          rrole.get('/' + pname + '/:ids', (req, res, next) => {
            co(function* () {
              ...
              yield send_res(req, res, name, 'item', data, title, con);
            }).catch(next);
          });
        }
        if (auth.new.includes(rname)) {
...
async function send_res(req, res, name, type, pdata, title, con) {
