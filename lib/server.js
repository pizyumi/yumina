...
async function add_web_router (roles, entities, con, e, d) {
  ...
  await pi.forEachSeries(roles, async (v) => {
    ...
    if (role.auto_generate_web) {
      ...
      rrole.get('/', (req, res, next) => {
        co(function* () {
          var data = {
            rname: rname,
            page: 'top',
            title: role.get_title(con)
          };
...
async function send_res(req, res, pdata, con) {
  var p1 = path.join(con.p_entity, pdata.name, pdata.type, 'page.ejs');
  if (!(await common.check_path(p1))) {
    p1 = path.join('front', 'type', pdata.type, 'page.ejs');
  }
  var p2 = path.join('front', 'main.ejs');
  var rdata = {
    username: req.user.name,
    user_id: req.user.id,
    js: pdata.name + '-' + pdata.type,
    ext: {
      ename: pdata.name,
      pname: pdata.pname,
      rurl: pdata.rurl
...
async function send_res_page(req, res, pdata, con) {
  var p1 = path.join('front', 'page', pdata.page, 'page.ejs');
  var p2 = path.join('front', 'main.ejs');
  var rdata = {
    username: req.user.name,
    user_id: req.user.id,
