          ...
          rrole.get('/' + pname, (req, res, next) => {
            co(function* () {
              if (_.result(req.query, 'action', '') === '') {
                yield send_res(req, res, rname, name, 'list', data, title, con);
              }
              else {
                next();
              }
        ...
        if (auth.item.includes(rname)) {
          ...
          rrole.get('/' + pname + '/:ids', (req, res, next) => {
            co(function* () {
              ...
              yield send_res(req, res, rname, name, 'item', data, title, con);
            }).catch(next);
          });
        }
        if (auth.new.includes(rname)) {
...
async function send_res(req, res, rname, name, type, pdata, title, con) {
  var p1 = path.join(con.p_entity, name, type, 'page.ejs');
