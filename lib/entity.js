...
obj = _.extend(obj, {
  ...
  get_entity_path: (name) => {
    return path.join(con.p_entity, name);
  },
  get_data_path: (name) => {
    return path.join(con.p_data, name + '.json');
  },
  get_data: async (name) => {
    return common.load_json_from_path(obj.get_data_path(name));
  },
  get_key_schema: async (schema) => {
    return _(schema).filter((v) => v.key);
  },
  get_not_key_schema: async (schema) => {
    return _(schema).filter((v) => !v.key);
  },
  ...
  get_table_key_column_names: async (entity) => {
    var n = entity.name;
    var s = entity.schema;
    var ks = await obj.get_key_schema(s);

    return await pi.mapSeries(ks, async (v) => await obj.get_table_column_name(v, n));
  },
  get_table_not_key_column_names: async (entity) => {
    var n = entity.name;
    var s = entity.schema;
    var nks = await obj.get_not_key_schema(s);

    return await pi.mapSeries(nks, async (v) => await obj.get_table_column_name(v, n));
  },
  generate_create_sql: async (entity) => {
    ...
    var columns = (await pi.mapSeries(s, async (v) => '\n  ' + await obj.get_table_column_name(v, n) + ' ' + await obj.get_table_column_type(v, n) + (v.key ? ' not null' : ''))).join(', ');
    var keys = (await pi.mapSeries(ks, async (v) => await obj.get_table_column_name(v, n))).join(', ');
  ...
  generate_insert_sql: async (entity, records) => {
    ...
    return (await pi.mapSeries(records, async (v) => {
      var values = (await pi.mapSeries(s, async (v2) => await obj.get_table_value_literal(v2, v[v2.name], n))).join(', ');
