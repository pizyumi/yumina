...
obj = _.extend(obj, {
  ...
  get_table_value_literal: (column, value, name) => {
    var func = get_custom_func(name, 'get_table_value_literal');
    if (func) {
      return func(column, value, name);
    }
    else {
      var t = column.type;
  ...
  generate_insert_sql: (entity, records) => {
    var n = entity.name;
    var s = entity.schema;

    var tname = obj.get_table_name(n);

    return _(records).map((v) => {
      var values = _(s).map((v2) => obj.get_table_value_literal(v2, v[v2.name], n)).join(', ');

      return `insert into ${tname} values (${values});`;
    }).join('\n') + '\n';
  },
  generate_delete_sql: (entity) => {
    var n = entity.name;
