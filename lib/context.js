...
obj = _.extend(obj, {
  get_context: async (env, p_work, p_yumina) => {
    ...
    var p_public = path.join(p_work, 'public');
    ...
    var p_temp = path.join(p_work, 'temp');
    ...
    var p_fsdata = conf.p_fsdata;
    var p_js = conf.p_js;
    var p_css = conf.p_css;

    var p_fstemp = path.join(p_fsdata, 'temp');
    var p_fspublic = path.join(p_fsdata, 'public');
